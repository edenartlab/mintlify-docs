...
A limitation of generative models is that they can only generate things they've been trained on. But what if you want to consistently compose with a specific object, person's face, or artistic style not found in the original training data? This is where *Concepts* come in.
...
Concepts are first trained by uploading example images to the [SDXL trainer](https://beta.eden.art/train/lora_trainer). Training takes around 10 minutes. Once trained, the concept becomes available in all endpoints, including images and video.

## Training

Train concepts through the [training UI](https://beta.eden.art/train/lora_trainer) or the [SDK](/guides/sdk).
...
- concept as a knight in shining armour (using angle brackets)
...
With style concepts, you don't need to reference the concept - just prompt normally and the style will be applied.
...
Once trained, select your concept in the [creation tool](https://beta.eden.art/create) and trigger it by name or `<concept>` in prompts.

## Exporting LoRAs

Eden concepts are compatible with other tools supporting LoRA.
...
<Note>
LoRA strength has relatively small effect because Eden concepts optimize token embeddings rather than LoRA matrices.
</Note>